...
func TestInstallCreatesSettingsWhenThereIsNone(t *testing.T) {
	...
	if err := NewInstaller(p).Install("claude-statusbar collect", false); err != nil {
...
func TestInstallRefusesToClobberAnExistingStatusLine(t *testing.T) {
	...
	err := NewInstaller(p).Install("claude-statusbar collect", false)
	if err == nil {
		t.Fatal("Install overwrote an existing statusLine, want a refusal")
	}
...
func TestInstallOverwritesWhenForced(t *testing.T) {
	...
	if err := NewInstaller(p).Install("claude-statusbar collect", true); err != nil {
		t.Fatalf("NewInstaller(p).Install(force): %v", err)
	}
...
func TestInstallRejectsUnparseableSettingsInsteadOfDestroyingThem(t *testing.T) {
	...
	if err := NewInstaller(p).Install("claude-statusbar collect", false); err == nil {
		t.Fatal("Install accepted corrupt settings, want an error")
	}
	b, _ := os.ReadFile(p)
